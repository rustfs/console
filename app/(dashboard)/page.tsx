...
export default function PerformancePage() {
  ...
  const storageBackend = useMemo(
    () => storageinfo?.backend ?? (storageinfo as { Backend?: StorageInfo["backend"] })?.Backend,
    [storageinfo]
  )
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- fetch once on mount; systemApi methods are stable
  }, [])
  ...
  if (error) {
    return (
      <Page>
        <PageHeader
          actions={
            <Button variant="outline" onClick={getPageData}>
