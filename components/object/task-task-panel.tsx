...
import { TaskTaskItem } from "./task-task-item"
import type { AnyTask } from "@/contexts/task-context"

interface TaskTaskPanelProps {
  tasks: AnyTask[]
  onClearTasks: () => void
}

export function TaskTaskPanel({ tasks, onClearTasks }: TaskTaskPanelProps) {
  ...
  const renderList = (list: AnyTask[]) => (
    <div className="flex flex-col gap-3">
      {list.length === 0 ? (
        ...
        list.map((task) => <TaskTaskItem key={task.id} task={task} />)
      )}
    </div>
  )
