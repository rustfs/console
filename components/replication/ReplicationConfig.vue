<template>
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between">
      <h2 class="text-lg font-medium">{{ t('Bucket Replication') }}</h2>
...
const loadReplication = async () => {
  ...
  loading.value = true
  try {
    const res = await getBucketReplication(props.bucketName)
...
const handleRowDelete = async (rule: ReplicationRule) => {
  ...
  try {
    if (remaining.length === 0) {
      await deleteBucketReplication(props.bucketName)
      await deleteRemoteReplicationTarget(props.bucketName, rule.Destination?.Bucket ?? '')
    } else {
      // 获取当前配置以获取 Role 字段
      const currentConfig = await getBucketReplication(props.bucketName)
      ...
      await putBucketReplication(props.bucketName, {
        Role: role,
        Rules: remaining,
      })
    }
