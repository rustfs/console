...
interface UserEditPoliciesProps {
  value: string[]
  options: OptionItem[]
  disabled?: boolean
...
export function UserEditPolicies({ value, options, disabled = false, onChange }: UserEditPoliciesProps) {
  const { t } = useTranslation()
  const [open, setOpen] = useState(false)

  const togglePolicy = (policy: string) => {
    if (value.includes(policy)) {
      onChange(value.filter((item) => item !== policy))
      return
    }
