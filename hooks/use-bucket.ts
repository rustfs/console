...
export function useBucket() {
  ...
  const createBucket = async (params: { Bucket: string; ObjectLockEnabledForBucket?: boolean }) => {
  ...
  const putBucketTagging = async (bucket: string, tagging: unknown) => {
    return client.send(new PutBucketTaggingCommand({ Bucket: bucket, Tagging: tagging as never }))
  }
  ...
  const putObjectLockConfiguration = async (bucket: string, objectLockConfiguration: unknown) => {
    return client.send(
  ...
  const putBucketLifecycleConfiguration = async (bucket: string, lifecycleConfiguration: unknown) => {
    return client.send(
  ...
  const putObjectRetention = async (bucket: string, key: string, retention: unknown) => {
    return client.send(new PutObjectRetentionCommand({ Bucket: bucket, Key: key, Retention: retention as never }))
  }
  ...
  const putBucketQuota = async (bucket: string, quota: unknown) => api.put(`/quota/${bucket}`, quota)
  const deleteBucketQuota = async (bucket: string) => api.delete(`/quota/${bucket}`)
  const getBucketQuotaUsage = async (bucket: string) => api.get(`/quota-stats/${bucket}`)
  const checkBucketQuota = async (bucket: string) => api.get(`/quota-check/${bucket}`)
