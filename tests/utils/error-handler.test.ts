...
describe('ConfigLoadError', () => {
  ...
  it('should support all error codes', () => {
    const codes: Array<ConfigLoadError['code']> = ['INVALID_URL', 'STORAGE_ERROR', 'NETWORK_ERROR', 'UNKNOWN_ERROR'];
...
describe('parseApiError', () => {
  it('should parse JSON error response', async () => {
    const response = new Response(JSON.stringify({ message: 'API Error' }), { status: 400, statusText: 'Bad Request' });
  ...
  it('should stringify JSON when no message property exists', async () => {
    ...
    const response = new Response(JSON.stringify(errorData), { status: 500 });
